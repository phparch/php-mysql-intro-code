      ...
      <div class="card-body">
        ...
        <form class="needs-validation" novalidate method="POST" action="<?= $_SERVER['PHP_SELF']; ?>">
          <div class="form-group">
            ...
            <input type="test" class="form-control" id="entered_cookie" name="entered_cookie" placeholder="Enter a cookie you like to eat" required>
          </div>
          <button type="submit" class="btn btn-primary" name="cookie_submission">Submit</button>
        </form>
        <?php
            }
            // User entered favorite cookie, save favorite cookie to Session variable and route to this page
        ...
        ?>
        <h4>
          Hey <?= $_SESSION['name'] ?>, you like <?= $_SESSION['favorite_cookie'] ?> cookies. Very nice!
        </h4>
        <?php
                session_destroy();  // End the current session
                $_SESSION = array(); // Destroy all session variables in the current session
            }
        ...
        <form class="needs-validation" novalidate method="POST" action="<?= $_SERVER['PHP_SELF']; ?>">
          <div class="form-group">
            ...
            <input type="test" class="form-control" id="entered_name" name="entered_name" placeholder="Enter your name" required>
          </div>
          <button type="submit" class="btn btn-primary" name="name_submission">Submit</button>
        </form>
        <?php
            }
        ?>
